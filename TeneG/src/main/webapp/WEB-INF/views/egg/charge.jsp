...
<script type="text/javascript">
	function onclickAmount() {
		...
		$("#total").html(amount + "&nbsp;<i class='fa fa-krw'></i>");
		$("#eggCount").html(amount/ 100 + " <img src='${ pageContext.request.contextPath }/img/egg2.png' style='width: 27px;height: 30px; margin-left: 3px;'/>");

	}

	function doCharge() {
		var amount = $(":input:radio[name=amount]:checked").val();
		var option = $(":input:radio[name=paymentOption]:checked").val();
		var ck1 = document.getElementsByName("join_user_agree");
		var ck2 = document.getElementsByName("join_priv_agree");
	...
	function changeCheck() {
		...
		if(ck1[0].checked && ck2[0].checked && amount && option){
			$("#chargeBtn").addClass("btn-success");
		}else{
			$("#chargeBtn").removeClass("btn-success");
		}
...
<body>
	...
	<section>
		<form name="cForm"
			action="<%=request.getContextPath()%>/egg/charge.do"
			method="post" onsubmit="return doCharge()">
			<input type="hidden" name="id" value="${loginUser.id }"/>
			<input type="hidden" name="password" value="${loginUser.password }"/>
			<div class="charge1" style="float: left;">
				<div class="chargeMenu">
					<label style="font-size: 21px; color: white;"> <spring:message
							code="amount" />
					</label>
				</div>
			<div class="chargeOption">
               <label> <input type="radio" class="amountRadio"
                  name="amount" value="10000" onclick="onclickAmount()" onchange="changeCheck()" >&nbsp;&nbsp;
                  10,000 <spring:message code="won" /></label>
                  <label><input type="radio"
                  class="amountRadio" name="amount" value="30000"
                  onclick="onclickAmount()" onchange="changeCheck()" style="margin-left: 48px">&nbsp;&nbsp;
                  30,000 <spring:message code="won" />
               </label><br /> <label> <input type="radio" name="amount"
                  class="amountRadio" value="50000" onclick="onclickAmount()" onchange="changeCheck()">&nbsp;&nbsp;
                  50,000 <spring:message code="won" /></label>
                  <label><input type="radio"
                  name="amount" class="amountRadio" value="100000"
                  onclick="onclickAmount()" onchange="changeCheck()" style="margin-left: 48px;">
                  100,000 <spring:message code="won" />
               </label><br /> <label> <input type="radio" name="amount"
                  class="amountRadio" value="150000" onclick="onclickAmount()" onchange="changeCheck()">&nbsp;
                  150,000 <spring:message code="won" /></label>
                  <label> <input type="radio"
                  class="amountRadio" name="amount" value="200000"
                  onclick="onclickAmount()"  onchange="changeCheck()" style="margin-left: 45px">
                  200,000 <spring:message code="won" />
               </label><br /> <br />
            </div>
				<div class="chargeMenu" style="margin-top: 50px;">
					<label style="font-size: 21px; color: white;"> <spring:message
							code="paymentOption" />
					</label>
				</div>
				<div class="chargeOption">
					<label> <input type="radio" name="paymentOption"
						value="카카오페이" onchange="changeCheck()"> <spring:message code="kakaoPay" /> <input
						type="radio" name="paymentOption" value="신용카드"
						style="margin-left: 44px;" onchange="changeCheck()"> <spring:message code="credit" />
					</label><br /> <label><input type="radio" name="paymentOption"
						value="무통장입금" onchange="changeCheck()"> <spring:message code="deposit" /> <input
						type="radio" name="paymentOption" value="휴대폰"
						style="margin-left: 40px;" onchange="changeCheck()"> <spring:message code="phone" />
					</label><br /> <label> <input type="radio" name="paymentOption" onchange="changeCheck()"
						value="실시간 계좌이체"> <spring:message code="account" />
					</label>
				</div>
				<div style="margin-top: 50px;">
					<ul>
						<li style="width: 350px; margin-left: 0px;">예치금 충전 시 신용카드
							전표/현금영수증은 개인 소득공제용으로만 사용이 가능합니다.</li>
						<li style="width: 350px; margin-left: 0px;">매입세액공제용 영수증이 필요하신
							경우 ‘기업전문가‘ 에게 서비스 구매 후, 세금계산서 발행을 받으시기 바랍니다.</li>
						<li style="width: 350px; margin-left: 0px;">예치금은 ‘통장’의 의미를
							가지고 있으며, 사이트 내 거래를 위해 편의상 보관되어 있는 금액이므로 세금계산서 발행이 불가합니다. (TeneG로의
							세금계산서 발행요청을 자제부탁드립니다.)</li>
					</ul>
				</div>
			</div>
			<div class="charge1">
				<div style="font-size: 21px;">
					<span><b> 충전될 Egg </b> </span> <span id="eggCount"
						style="float: right;"> 0 <img
						src="${ pageContext.request.contextPath }/img/egg2.png" style="width: 27px;height: 30px; margin-left: 3px;" /></span>

				</div>
				<div style="font-size: 21px; margin-top: 50px;">
					<span><b> 총 결제할 금액 </b> </span> <span id="total"
						style="float: right; margin-right:3px;"> 0 &nbsp;<i class="fa fa-krw"></i>
					</span>
							...
							<div class="panel panel-default">
								...
								<div class="panel-body">
									<div class="form-group pull-left">
										<label class="control-label"> 결제 약관 </label>
										<div class="col-xs-12">
											<textarea class="form-control" readonly rows="10" cols="80">서비스약관
										...
										<div class="col-xs-12">
											<div class="checkbox pull-right">
												<label for="join_user_agree"> <input type="checkbox"
													name="join_user_agree" id="join_user_agree" value="1" onchange="changeCheck()" />
													결제 약관에 동의합니다.
												</label>
											</div>
										</div>
...
회사는 개인정보 수집 및 이용목적이 달성된 후에는 예외 없이 해당 정보를 지체 없이 파기합니다.
										...
										<div class="col-xs-12">
											<div class="checkbox pull-right">
												<label for="join_priv_agree"> <input type="checkbox"
													name="join_priv_agree" id="join_priv_agree" value="1" onchange="changeCheck()" />
													개인정보 처리방침에 동의합니다.
												</label>
											</div>
										</div>
							...
							<input type="submit" class="btn" id="chargeBtn"
								value="<spring:message code="charge" />"
								style="width: 60%; margin: 20px auto auto 150px;">
						</div>
					</div>
				</div>
			</div>
